...
std::string ImageConverter::name() const {
    return "Image Converter";
}

std::vector<std::pair<FileType, FileType>> ImageConverter::supportedFormats() const {
    return {
...
std::string ImageConverter::validate(const std::string& inputPath, FileType outputType) {
    FileType inputType = Detector::detect(inputPath);
    return Validator::validate(inputPath, inputType, outputType);
}

bool ImageConverter::convert(const std::string& inputPath, const std::string& outputPath,
                             FileType outputType) {
    (void)outputType;
    try {
